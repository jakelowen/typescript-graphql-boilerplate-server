...
import { noPermissionByName, noTeamExists, noUserExists, duplicatePermission, notTeamAdminError } from "./logic/errors";
...
describe("addPermissionMutation", () => {
  test("can not add if not team admin", async () => {
    ...
    const permission = { id: faker.random.number(), name: faker.random.word().toUpperCase() };
    ...
    const response = await testClient.client.mutate({
      ...
      `,
      variables: { input: { permissionName: permission.name, userId: user.id, teamId: team.id } }
    });
    expect((response.data as any).addPermission.error.length).toBe(1)
    expect((response.data as any).addPermission.error[0].message).toEqual(notTeamAdminError)
  });
  ...
  test("works", async () => {
    ...
    const response = await testClient.client.mutate({
      ...
      `,
      variables: { input: { permissionName: permissions[0].name, userId: users[0].id, teamId: team.id } }
    });
    expect((response.data as any).addPermission.error).toBeNull()
    expect((response.data as any).addPermission.permission.name).toEqual(permissions[0].name)
    expect((response.data as any).addPermission.permission.team.id).toEqual(team.id)
  });
  ...
  test("can not apply duplicate permission", async () => {
    ...
    expect((response.data as any).addPermission.error.length).toBe(1)
    expect((response.data as any).addPermission.error[0].message).toEqual(duplicatePermission)
  ...
  test("user does not exist error", async () => {
    ...
    const response = await testClient.client.mutate({
      ...
      `,
      variables: { input: { permissionName: permission.name, userId: faker.random.uuid(), teamId: team.id } }
    });
    expect((response.data as any).addPermission.error.length).toBe(1)
    expect((response.data as any).addPermission.error[0].message).toEqual(noUserExists)
  });
  ...
  test("team does not exist error", async () => {
    ...
    const response = await testClient.client.mutate({
      ...
      `,
      variables: { input: { permissionName: permission.name, userId: faker.random.uuid(), teamId: faker.random.uuid() } }
    });
    expect((response.data as any).addPermission.error.length).toBe(1)
    expect((response.data as any).addPermission.error[0].message).toEqual(noTeamExists)
  });
  ...
  test("permission does not exist error", async () => {
    ...
    const response = await testClient.client.mutate({
      ...
      `,
      variables: { input: { permissionName: faker.random.word(), userId: faker.random.uuid(), teamId: faker.random.uuid() } }
    });
    expect((response.data as any).addPermission.error.length).toBe(1)
    expect((response.data as any).addPermission.error[0].message).toEqual(noPermissionByName)
});